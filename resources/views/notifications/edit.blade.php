...
<head>
    <meta charset="UTF-8">
    <title>Modifier la notification</title>
...
<body>
    ...
    <form action="{{ route('notifications.update', $notification) }}" method="POST">
        ...
        <input id="type" name="type" value="{{ old('type', $notification->type) }}" required>

        <label for="contenu">Contenu</label>
        <textarea id="contenu" name="contenu" required>{{ old('contenu', $notification->contenu) }}</textarea>

        <label for="date_creation">Date de création</label>
        <input id="date_creation" name="date_creation" type="date" value="{{ old('date_creation', $notification->date_creation) }}" required>

        <label for="vue">Vue</label>
        <select id="vue" name="vue" required>
            <option value="1" {{ old('vue', $notification->vue) ? 'selected' : '' }}>Oui</option>
            <option value="0" {{ !old('vue', $notification->vue) ? 'selected' : '' }}>Non</option>
        </select>

        <button type="submit">Enregistrer les modifications</button>
    </form>

    <p><a href="{{ route('notifications.show', $notification) }}">Retour à la notification</a></p>
</body>
</html>
