...
<head>
    <meta charset="UTF-8">
    <title>Ajouter un abonnement premium</title>
...
<body>
    <h1>Ajouter un abonnement premium</h1>
    ...
    <form action="{{ route('abonnements-premium.store') }}" method="POST">
        @csrf
        ...
        <select id="vendeur_id" name="vendeur_id" required>
            ...
            @foreach($vendeurs as $vendeur)
                <option value="{{ $vendeur->id }}" {{ old('vendeur_id') == $vendeur->id ? 'selected' : '' }}>{{ optional($vendeur->user)->name ?? 'Vendeur #' . $vendeur->id }}</option>
            @endforeach
        </select>
        ...
        <input id="date_fin" name="date_fin" type="date" value="{{ old('date_fin') }}">

        <label for="est_actif">Actif</label>
        <select id="est_actif" name="est_actif" required>
            <option value="1" {{ old('est_actif') == '1' ? 'selected' : '' }}>Oui</option>
            <option value="0" {{ old('est_actif') == '0' ? 'selected' : '' }}>Non</option>
        </select>

        <button type="submit">Enregistrer</button>
    </form>

    <p><a href="{{ route('abonnements-premium.index') }}">Retour à la liste des abonnements premium</a></p>
</body>
</html>
