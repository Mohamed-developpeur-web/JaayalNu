...
<head>
    <meta charset="UTF-8">
    <title>Modifier le vendeur</title>
    ...
    <form action="{{ route('vendeurs.update', $vendeur) }}" method="POST">
        ...
        <select id="user_id" name="user_id" required>
            @foreach($users as $user)
                <option value="{{ $user->id }}" {{ old('user_id', $vendeur->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }})</option>
            @endforeach
        </select>

        <label for="status_compte">Statut du compte</label>
        <input id="status_compte" name="status_compte" value="{{ old('status_compte', $vendeur->status_compte) }}" required>

        <label for="est_premium">Est premium</label>
        <select id="est_premium" name="est_premium" required>
            <option value="1" {{ old('est_premium', $vendeur->est_premium) ? 'selected' : '' }}>Oui</option>
            <option value="0" {{ !old('est_premium', $vendeur->est_premium) ? 'selected' : '' }}>Non</option>
        </select>

        <button type="submit">Enregistrer les modifications</button>
    </form>

    <p><a href="{{ route('vendeurs.show', $vendeur) }}">Retour au vendeur</a></p>
</body>
</html>
